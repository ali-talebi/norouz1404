...
for iter_alpha in total_alpha_content:
    ...
    for iter_flat in total_flat_content:
        ...
        x_u_train, x_u_test, y_u_train, y_u_test = train_test_split(stand_df_with_out,
                                                                    concat_2_df_health_fault['class'], random_state=42,
                                                                    test_size=0.3)
...
plt.figure(figsize=(5, 5))
for el in for_each_generate_health:
    ax1 = plt.scatter(el[0], el[1], c='blue', alpha=0.1)

for e3 in for_each_generate_fault_t:
    ax4 = plt.scatter(e3[0], e3[1], c='red', alpha=0.1)

plt.legend(('health', 'fault'))
plt.title("Overlaping Health and Fault [blue is Health and Red in Fault ] ")
plt.grid()
plt.show()

plt.figure(figsize=(10, 10))
for el in for_each_generate_health:
    ax1 = plt.plot(el[0], el[1], c='b', alpha=0.1)
for el in for_each_generate_health_t:
    ax2 = plt.plot(el[0], el[1], c='r', alpha=0.1)
for e2 in for_each_generate_fault:
    ax3 = plt.plot(e2[0], e2[1], c='g', alpha=0.1)
for e3 in for_each_generate_fault_t:
    ax4 = plt.plot(e3[0], e3[1], c='yellow', alpha=0.1)
...
plt.figure(figsize=(10, 10))
for e2 in for_each_generate_fault:
    ax3 = plt.plot(e2[0], e2[1], c='green', alpha=0.1)
for e3 in for_each_generate_fault_t:
    ax4 = plt.plot(e3[0], e3[1], c='orange', alpha=0.1)

plt.legend()
plt.grid()
plt.title("orange is alpha + void , green is fault void ")
plt.show()
...
plt.title("blue is health and red is void ")
plt.legend()
plt.grid()
plt.show()
...
for el in for_each_generate_health:
    ax1 = plt.scatter(el[0], el[1], c='b', alpha=0.1)
for e3 in for_each_generate_fault_t:
    ax4 = plt.scatter(e3[0], e3[1], c='orange', alpha=0.1)

plt.legend()
plt.title("Health is blue , alpha + void is orange ")
plt.grid()
plt.show()
